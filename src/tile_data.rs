...
impl TileData {
    pub fn apply(&mut self, src: EntityRef) {
        self.components
            .values_mut()
            .filter_map(|(refl, value)| refl.reflect(src).map(|new_value| (value, new_value)))
            .for_each(|(value, new_value)| value.apply(new_value))
    }

    pub fn insert(&self, entity: &mut EntityMut) {
        self.components
            .values()
            .for_each(|(refl, component)| refl.insert(entity, component.as_ref()))
    }

    pub fn remove(&self, entity: &mut EntityMut) {
        self.components
            .values()
            .for_each(|(refl, _)| refl.remove(entity))
    }
...
impl<'a> EditorTileDataRegistryLock<'a> {
    pub(crate) fn access_tileset_data(
        ...
        self.0.map.entry(tileset_info).or_default()
    }
...
pub struct TileDataAccess<'a>(&'a mut TileData, &'a AppTypeRegistry);

impl<'a> TileDataAccess<'a> {
    pub fn remove<T: Reflect + Typed>(&mut self) -> &mut Self {
        self.0.components.remove(&TypeId::of::<T>());

        self
    }

    pub fn insert<T: Reflect + Typed>(&mut self, data: T) -> Result<&mut Self, EditorError> {
        let (type_id, reflect_component) = Self::get_type_data::<T>(self.1)?;

        self.0
            .components
            .insert(type_id, (reflect_component, Box::new(data)));

        Ok(self)
    }

    fn get_type_data<T: Reflect + Typed>(
        registry: &AppTypeRegistry,
    ) -> Result<(TypeId, ReflectComponent), EditorError> {
        let ty_name = <T as Typed>::type_info().type_name();
        let type_id = TypeId::of::<T>();

        let registry_lock = registry.read();
        let reflect_component = registry_lock
            .get(type_id)
            .ok_or(EditorError::TypeNotRegistered { ty_name })?
            .data::<ReflectComponent>()
            .ok_or(EditorError::TypeNotReflectComponent { ty_name })?
            .to_owned();
